  ...
  return (
    <div className="article-card">
      {article.image_url ? (
        ...
        <div className="article-card-img" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '2.5rem', background: 'var(--f1-dark)' }}>⚽</div>
      )}
    ...
    <div>
      {scorers.slice(0, 5).map((s, i) => (
        <div key={s.player_id} className="scorer-row" style={{ padding: '0.5rem 0' }}>
          ...
          <div className="scorer-avatar" style={{ width: '36px', height: '36px', fontSize: '1rem' }}>⚽</div>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
          ...
          {!loading && standings.length > 0 && (
            <div style={{
              ...
              {[
                { label: 'Leader', value: standings[0]?.team_name || '–', icon: '🏆' },
                { label: 'Matchs joués', value: Math.round(standings.reduce((a, s) => a + s.played, 0) / 2), icon: '⚽' },
                { label: 'Clubs', value: standings.length, icon: '🛡️' },
                { label: 'Meilleur buteur', value: topScorers[0] ? `${topScorers[0].player_name} (${topScorers[0].goals} buts)` : '–', icon: '🎯' },
