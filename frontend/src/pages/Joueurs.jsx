...
const POSTE_ICON = {
  Gardien: '🧤', Défenseur: '🛡️', Milieu: '⚙️', Attaquant: '⚡',
}
            ...
            {topOnly.map((p, i) => (
              <div key={p.id} className="scorer-row">
                ...
                <div className="scorer-info">
                  <div className="scorer-name">{p.nom}</div>
                  <div className="scorer-team">
                    {POSTE_ICON[p.poste] || ''} {p.poste} · {p.team?.nom || teamName(p.team_id)}
                  </div>
                </div>
                  ...
                  <tr key={p.id}>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <span style={{ fontSize: '1.2rem' }}>{POSTE_ICON[p.poste] || '👤'}</span>
                        <span style={{ fontWeight: '700', color: 'var(--f1-dark)' }}>{p.nom}</span>
                      </div>
                    </td>
                    <td>
